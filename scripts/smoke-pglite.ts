...
async function main(): Promise<void> {
  ...
  const storedJob = (await db.select().from(t.importJobs).where(eq(t.importJobs.id, addJob.id)))[0];
  check("import_jobs.additions jsonb round-trip",
    storedJob?.additions?.enrollments[0]?.programId === second
    && Array.isArray(storedJob?.additions?.serviceLogIds));
